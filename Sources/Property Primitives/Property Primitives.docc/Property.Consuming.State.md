...
The class carries `@unchecked Sendable` so that the outer
`Property.Consuming: Sendable where Base: Sendable` conditional conformance
can propagate through the reference-type `State` field without
over-constraining all `Consuming` instantiations. See the `WORKAROUND` block
in `Property.Consuming.State.swift` for the tracking details, including the
trigger condition for removing the workaround.
